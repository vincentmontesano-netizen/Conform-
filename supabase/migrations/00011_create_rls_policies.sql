...
CREATE POLICY "admin_update_company" ON public.companies
  FOR UPDATE USING (id = auth.user_company_id() AND auth.user_role() = 'admin');
...
CREATE POLICY "users_select_company_profiles" ON public.profiles
  FOR SELECT USING (company_id = auth.user_company_id() OR user_id = auth.uid());

CREATE POLICY "admin_insert_profiles" ON public.profiles
  FOR INSERT WITH CHECK (auth.user_role() = 'admin' OR user_id = auth.uid());

CREATE POLICY "admin_update_profiles" ON public.profiles
  FOR UPDATE USING (auth.user_role() = 'admin' OR user_id = auth.uid());
...
CREATE POLICY "users_select_company_versions" ON public.duerp_versions
  FOR SELECT USING (
    duerp_id IN (SELECT id FROM public.duerp_documents WHERE company_id = auth.user_company_id())
  );
...
CREATE POLICY "write_roles_manage_alerts" ON public.compliance_alerts
  FOR UPDATE USING (
    company_id = auth.user_company_id()
    AND auth.user_role() IN ('admin', 'rh')
  );
